...
def distr_binom ():
    ...
    p= proobabilidade de ocorrencia do evento\n
    x= numero de itens selecionados (porem só é usado para elevar P ou (1-P)**(n-x))\n

    '''
    entrada= input('Digite as entradas\nExemplo n;x;p;\n\nEntrada:')

    n, x, p, x=entrada.split(';')
    n, x, = map(int, (n, x,))
    p= float(p)
